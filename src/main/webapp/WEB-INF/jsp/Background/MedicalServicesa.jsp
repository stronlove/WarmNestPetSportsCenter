...
<!-- 表格 -->
	<table class="table table-striped"
		...
		<c:if test="${clist=='[]' }">
			<tr>
			<th colspan="6"><h1>您暂时没有医疗服务，请查询取药</h1></th>
			</tr>
		</c:if>
		</tbody>
	</table>
	<c:if test="${clist[0].tstate==0 }">
		<button style="position: absolute;left: 700px;top: 80px;width: 150px;" class="btn btn-info" onclick="Getthemedicine('${clist[0].topenlists.openlistno}')">确认治疗</button>
	</c:if>
	<c:if test="${clist[0].tstate==1 }">
		<button style="position: absolute;left: 700px;top: 80px;width: 150px;background: black;" class="btn btn-info">已治疗</button>
	</c:if>
