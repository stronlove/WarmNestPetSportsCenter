...
<html>
  <head>
    ...
    <title>My JSP 'MedicalServicesa.jsp' starting page</title>
...
<!-- 表格 -->
	<table class="table table-striped"
		...
		<c:if test="${clist=='[]' }">
			<tr>
			<th colspan="6"><h1>您暂时没有要付的钱，请查询用户付钱</h1></th>
			</tr>
		</c:if>
		</tbody>
	</table>
	<c:if test="${clist[0].tstate==1 }">
		<button style="position: absolute;left: 700px;top: 80px;width: 150px;" class="btn btn-info" onclick="Getthemedicine('${clist[0].topenlists.openlistno}')">确认付钱</button>
	</c:if>
	<c:if test="${clist[0].tstate==0 }">
		<button style="position: absolute;left: 700px;top: 80px;width: 150px;background: black;" class="btn btn-info">已付钱</button>
	</c:if>
	...
	<script type="text/javascript">
		function Getthemedicine(openlistno){
			window.location.href="yuefuqian?openlistno="+openlistno;
		}
