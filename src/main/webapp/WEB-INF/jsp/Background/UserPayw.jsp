			...
			<c:if test="${pm.rows.size()>0 }">
				<c:forEach items="${pm.rows}" var="l">
					<tr>
						...
						<td><a href="Commodityselects?openlistno=${l.openlistno }" class="btn btn-info">查看详情</a></td>
					</tr>
				</c:forEach>
			</c:if>
           <tr>
               <td colspan="6">
	               <pg:pager url="UserPayw"  items="${pm.total}"  export="currentPageNumber=pageNumber"   maxPageItems="${pm.pagesize}" >
